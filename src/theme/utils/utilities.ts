...
import { directionColumRowMap,directionMap } from './mapping';
...
/**
 * Genera las entradas CSS para el gap en función de la dirección y el número proporcionados.
 *
 * @param params - Array que contiene [_, dirección, número]
 * @returns Una entrada CSS para el padding o undefined
 */
export const gapSpacing = ([, direction, number]: string[]): CSSEntries | undefined => {
  if (!number) return undefined;

  const value = +number;

  // Calcular el valor del gap en rem
  const remValue = `${value * 0.5}rem`;

  // Calcular el valor del gap en vh
  const vhValue = value <= 2 ? '1vh' : `${value - 1}vh`;

  // Mapear la dirección a la propiedad CSS correcta
  const property = direction ? `${directionColumRowMap[direction]}-gap` : 'gap';

  // Devolver la entrada CSS para el gap con min
  return [[property, `min(${remValue}, ${vhValue})`]];
};
