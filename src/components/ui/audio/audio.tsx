...
import { Pause, PersonStanding, Play, Volume1, Volume2, VolumeOff } from 'lucide-react';
...
export const Audio: React.FC<AudioProps> = ({
   ...
   * @param {React.RefObject<HTMLButtonElement>} refVolumeButton - La referencia al botón de volumen.
   * @returns {boolean} Verdadero si se debe cerrar el menú, falso de lo contrario.
   */
  const shouldCloseOnInteractOutside = (
    element: HTMLElement,
    refVolumeButton: React.RefObject<HTMLButtonElement | null>
  ) => {
  ...
  const onLoadedMetadata = (): void => {
    if (!refAudio.current) return;
    const isTouchScreen: boolean = window.matchMedia('(any-hover: none) and (any-pointer: coarse)').matches;

    setDuration(Math.round(refAudio.current.duration));
    setVolume(isTouchScreen ? 1 : refAudio.current.volume);
  };
  ...
  const getVolumeIcon = () => {
    if (volume === VOLUME_OFF_THRESHOLD) {
      return <VolumeOff className="h-4 w-4 fill-[var(--audio-icon-color)]" />;
    } else if (volume <= VOLUME_LOW_THRESHOLD) {
      return <Volume1 className="h-4 w-4 fill-[var(--audio-icon-color)]" />;
    } else {
      return <Volume2 className="h-4 w-4 fill-[var(--audio-icon-color)]" />;
    }
  };
  ...
  return (
    <>
      <div
        className={cn(`${css['c-audio__bar']} ${css[`c-audio__bar--${size}`]}`, { [className ?? '']: className })}
        role="group"
        ...
        {a11y ? (
          <div className="inline-block border border-[var(--audio-icon-color)] rounded-full">
            <PersonStanding className="h-4 w-4" />
          </div>
        ) : null}

        <button type="button" onClick={togglePlay}>
          <span className="sr-only">{play ? 'Pausar' : 'Reproducir'}</span>
          {play ? (
            <Pause className="h-4 w-4 fill-[var(--audio-icon-color)]" />
          ) : (
            <Play className="h-4 w-4 fill-[var(--audio-icon-color)]" />
          )}
        </button>
        ...
        <label className="sr-only" htmlFor={`time${uid}`}>
          Tiempo transcurrido
        </label>
        <input
          className={`${css['c-audio__bar-scrubber']}`}
          id={`time${uid}`}
          value={mediaTime}
          min={0}
          max={duration}
        ...
        <button
          ...
          className={css['c-audio__bar-volume-btn']}
          onClick={() => setOpenMenu(!openMenu)}>
          <span className="sr-only">Controlar volumen</span>
          {getVolumeIcon()}
        </button>
        {openMenu ? (
          <div className={css['c-audio__bar-volume-control']} ref={refVolumeSlider}>
            <label className="sr-only" htmlFor={`volume${uid}`}>
              Volumen
            </label>
            <input
              id={`volume${uid}`}
      ...
      <audio
        ...
        preload="metadata"
        controls
        className={cn(`${css['c-audio']}`, { [className ?? '']: className })}
